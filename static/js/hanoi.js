    ...
    */
    function init(disks){
        ...
        for (var i = 0; i < disks; i++){
            $('#disks').append('<div class="disk"><img class="colorbox" src="static/assets/disk_bleu.png"></div>');
        }

        disk = $(".disk").toArray().reverse();
        pillarA = disk;
        $('#columns').append('<div class="column"><div class="colShadow"></div></div><div class="column"><div class="colShadow"></div></div><div class="column"><div class="colShadow"></div></div>');
    ...
    function diskSquash(){
        ...
        for (var i = 0; i < disk.length; i++) {
            ...
            $(disk[i]).css({'padding-left': padding+"px", 'padding-right': padding+"px"});
            $(disk[i]).children().attr({'src': ('static/assets/disk_'+ COLORS[(i % COLORS.length)] +'.png')});
        }
    }
    ...
    */
    ...
    $('.fa-moon').click(function(){
        ...
        if(color == 'rgb(20, 20, 20)'){
            ...
            $('html').css({'background': 'rgb(20, 20, 20)', 'filter': 'invert(1) hue-rotate(180deg)'});
        }
    ...
    function move(from, target){
        ...
        switch(target) {
            case "A":
                ...
                $(disk).animate({bottom: "+="+(pillarSize - originHeight + 4 )+"em"}, 200 * time);
                $(disk).delay(200 * time).animate({marginLeft: "0"}, 200 * time);
                $(disk).delay(400 * time).animate({bottom: "-="+(pillarSize - targetHeight + 1 )+"em"}, 200 * time);
                break;
            ...
            case "B":
                ...
                $(disk).animate({bottom: "+="+(pillarSize - originHeight + 4 )+"em"}, 200 * time);
                $(disk).delay(200 * time).animate({marginLeft: "33%"}, 200 * time);
                $(disk).delay(400 * time).animate({bottom: "-="+(pillarSize - targetHeight + 1 )+"em"}, 200 * time);
                break;
            ...
            case "C":
                ...
                $(disk).animate({bottom: "+="+(pillarSize - originHeight + 4 )+"em"}, 200 * time);
                $(disk).delay(200 * time).animate({marginLeft: "66%"}, 200 * time);
                $(disk).delay(400 * time).animate({bottom: "-="+(pillarSize - targetHeight + 1 )+"em"}, 200 * time);
                break;
        }
    }
